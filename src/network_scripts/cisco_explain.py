...
def parse_dump_input(text: str) -> DumpInput:
    """Return transcript text plus network-scripts metadata when present."""
    normalized = text.replace("\r\n", "\n")
    metadata: dict[str, str] = {}
    if normalized.startswith("--- network-scripts dump metadata ---\n"):
        header, separator, transcript = normalized.partition(f"{TRANSCRIPT_DELIMITER}\n")
        if separator:
            ...
            return DumpInput(transcript=transcript, metadata=metadata)
    return DumpInput(transcript=text, metadata=metadata)
    ...
    for key, name, intro in CATEGORIES:
        ...
        for b in cat_blocks:
            if key == "interfaces" and b.header.startswith("interface "):
                ...
                card_rows = [("Sub-setting", html.escape(ln.strip())) for ln in b.lines] if b.lines else None
                section_parts.append(render_card(b.header, what, card_rows, b.text()))
...
# ---------------------------------------------------------------------------
# Public API and script entry point
# ---------------------------------------------------------------------------
...
def explain_dump(input_path: Path, output_path: Path | None = None) -> ExplainResult:
    src = Path(input_path)
    dump_input = parse_dump_input(src.read_text(encoding="utf-8", errors="replace"))
    transcript = dump_input.transcript
    hostname = dump_input.metadata.get("hostname") or find_hostname(transcript)
...
def main() -> int:
    ...
    print(f"  model:      {result.version.model}")
    print(
